...
class CastClient {
  static async find(name) {
    const devices = await mdns.discover({
      ...
      filter: (device) => {
        return device.familyName === name || device.address === name;
      },
    });
    if (devices.length === 0) {
      throw Error('Cannot find any devices matching the name: ' + name);
    }
  ...
  async stop() {
    await promisifyAndCall(this._client, this._client.stop);
  }
